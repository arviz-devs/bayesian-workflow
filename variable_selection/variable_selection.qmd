...
ax1 = axes[0]
ax1.hist(student_Gmat['Gmat'], bins=np.sort(student_Gmat['Gmat'].unique()))
ax1.set(xlabel="Median math exam score", yticks=[])
ax1.spines[['top', 'right']].set_visible(False)

ax2 = axes[1]
ax2.hist(student_Gpor['Gpor'], bins=np.sort(student_Gpor['Gpor'].unique()))
...
Before variable selection, we want to build a good model with all covariates. We first illustrate that very wide priors may be bad when we have many predictors. By default Bambi uses very weakly informative priors on regression coefficients.
...
fitm_u.predict(fitm_u_idata, kind="pps", inplace=True)
```
...
mod_n1 = bmb.Model(formula, data=studentstd_Gmat,
                    priors={'common': bmb.Prior('Normal', mu=0, sigma=2.5)})

mod_n1_res = mod_n1.fit(draws=4000, tune=1000, random_seed=42, idata_kwargs={"log_likelihood": True})
...
mod_n1pt = bmb.Model(formula, data=studentstd_Gmat,
                       priors={
                           'common': bmb.Prior('Normal', mu=0, sigma=2.5),
                           'sigma': bmb.Prior('TruncatedStudentT', 
                                             nu=3, mu=0, sigma=3,
                                             lower=5,
                                             dist=TruncatedStudentT)
                       })
mod_n1pt.build()
...
# Sample from posterior with normal(0, 2.5) prior
mod_n2 = bmb.Model(formula, data=studentstd_Gmat,
                    priors={'common': bmb.Prior('Normal', mu=0, sigma=scale_b)})

mod_n2_res = mod_n2.fit(draws=4000, tune=1000, random_seed=42, idata_kwargs={"log_likelihood": True})
...
mod_hs_res = mod_hs.fit(draws=4000, tune=1000, random_seed=42, idata_kwargs={"log_likelihood": True})
...
mod_r2d2_res = mod_r2d2.fit(draws=4000, tune=1000, random_seed=42, idata_kwargs={"log_likelihood": True})
...
bayesian_r2_priors = az.from_dict({"ff": {
    name: az.bayesian_r2(idata,
                         group="prior",
                         pred_mean="mu",
                         scale="sigma",
                         summary=False)
...
pc.get_viz("plot", "priors").set_xlim(0.04,.42)
...
bayesian_r2_posteriors = az.from_dict({"ff": {
    name: az.bayesian_r2(idata,
                         pred_mean="mu",
                         scale="sigma",
                         summary=False)
...
loo_r2_posteriors = az.from_dict({"ff": {
    name: az.loo_r2(idata,
                    var_name="Gmat",
                    summary=False)
    for name, idata in models.items()
    }}, sample_dims=["draw"])
...
mod_r2d2_por_res = mod_r2d2_por.fit(draws=4000, tune=1000, random_seed=42, idata_kwargs={"log_likelihood": True})
...
```{python}
pc_por = kpt.plot_forest(ppi_por,
                   submodels=[8],
                  );
