...
Data plotted with base Matplotlib
...
Stan model 0 (without priors). Compile the Stan model code using pedantic mode
...
print_stan(mod0.code())
mod0
```

Stan model 1 (with priors). Compile the updated Stan model code using pedantic mode
...
PLot 20 logistic curves given 20 posterior
_draws_, and one logistic given the posterior mean

```{python}
# | label: fig-bioassay-posterior-plot
posterior = az.extract(dt_bioassay1, group="posterior")
posterior_20 = az.extract(dt_bioassay1, group="posterior", num_samples=20, random_seed=SEED, show_progress=False)
...
```{python}
az.summary(idata_bambi, var_names="~p")
```

With `bambi` we can the built-in plotting functions in the interpret submodule.
