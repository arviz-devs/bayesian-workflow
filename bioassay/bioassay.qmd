...
image: ../social_cards/bioassay.png
...
with pm.Model() as mod0:
    a = pm.Flat("a")
    b = pm.Flat("b")
    pm.Binomial("y", n=N, logit_p=a + b * x, observed=y)
```
...
```{.python}
with mod1:
    dt_bioassay1 = pm.sample(random_seed=SEED)
```
:::
...
plot = bmb.interpret.plot_predictions(model, idata_bambi, "dose",
                                      use_hdi=False,
                                      fig_kwargs={"theme": {"figure.figsize": (8, 3)}})
