...
_, ax = plt.subplots()
ax.set_xlim(-R_plot, dist + 3 * R_plot)
...
ax.set(xticks=[-2, 0, 2], xticklabels=[r"$-2\sigma$", "0", r"$2\sigma$"])
...
```{python}
az.summary(az.from_cmdstanpy(fit_8), var_names=["distance_tolerance", "overshot"])
```
...
def compute_loo(fit, gq_model, data):
    ...
    return az.loo(az.from_cmdstanpy(gq, log_likelihood="log_lik"))

loo_6 = compute_loo(fit_6, gq_ll, golf_new_data)
loo_7 = compute_loo(fit_7, gq_ll, golf_new_data)
loo_8 = compute_loo(fit_8, gq_ll, golf_new_data)
```
