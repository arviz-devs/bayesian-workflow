...
```{python}
bambi_kwargs ={"random_seed": SEED, "idata_kwargs": {"log_likelihood": True}, "inference_method":"nutpie"}
```
...
#| results: hide
#| cache: true
fit_m1 = bmb.Model("kcal.per.g ~ 1", milk)
dt_m1 = fit_m1.fit(**bambi_kwargs)

fit_m2 = bmb.Model("kcal.per.g ~ neocortex", milk)
dt_m2 = fit_m2.fit(**bambi_kwargs)

fit_m3 = bmb.Model("kcal.per.g ~ log_mass", milk)
dt_m3 = fit_m3.fit(**bambi_kwargs)

fit_m4 = bmb.Model("kcal.per.g ~ neocortex + log_mass", milk)
dt_m4 = fit_m4.fit(**bambi_kwargs)
```
...
```{python}
comp_milk_w = comp_milk[["elpd_diff", "dse"]].copy()
comp_milk_w["p_worse"] = pz.Normal(comp_milk_w["elpd_diff"], comp_milk_w["dse"]).cdf(0)
comp_milk_w.round(2)
```
...
comp_milk_b = comp_milk_ref[["elpd_diff", "dse"]].copy()
comp_milk_b["p_better"] = pz.Normal(-comp_milk_b["elpd_diff"], comp_milk_b["dse"]).cdf(0)
comp_milk_b.round(2)
```
...
#| results: hide
#| cache: true
sleepstudy2["Subject"] = sleepstudy2["Subject"].astype(str)

fit_s1 = bmb.Model("Reaction ~ Days", sleepstudy2, family="gaussian")
idata_s1 = fit_s1.fit(**bambi_kwargs)

fit_s2 = bmb.Model("Reaction ~ Days + (1 | Subject)", sleepstudy2, family="gaussian")
idata_s2 = fit_s2.fit(**bambi_kwargs)

fit_s3 = bmb.Model("Reaction ~ Days + (Days | Subject)", sleepstudy2, family="gaussian")
idata_s3 = fit_s3.fit(**bambi_kwargs)

fit_s2.predict(idata_s2, kind="response")
fit_s3.predict(idata_s3, kind="response")
```
...
comp_sleep_df = comp_sleep[["elpd_diff", "dse"]].copy()
comp_sleep_df["p_worse"] = pz.Normal(comp_sleep_df["elpd_diff"], comp_sleep_df["dse"]).cdf(0)
comp_sleep_df.round(2)
```
...
pc = az.plot_loo_interval(idata_s2,
                        visuals={"ylabel": {"text": "Reaction time (ms)"}})
```
...
#| results: hide
#| cache: true
fit_s1t = bmb.Model("Reaction ~ Days", sleepstudy2, family="t")
idata_s1t = fit_s1t.fit(**bambi_kwargs)

fit_s2t = bmb.Model("Reaction ~ Days + (1 | Subject)", sleepstudy2, family="t")
idata_s2t = fit_s2t.fit(**bambi_kwargs)

fit_s3t = bmb.Model("Reaction ~ Days + (Days | Subject)", sleepstudy2, family="t")
idata_s3t = fit_s3t.fit(**bambi_kwargs)


fit_s2t.predict(idata_s2t, kind="response")
fit_s3t.predict(idata_s3t, kind="response")
```
...
comp_s3_s3t_df = comp_s3_s3t[["elpd_diff", "dse"]].copy()
comp_s3_s3t_df["p_worse"] = pz.Normal(comp_s3_s3t_df["elpd_diff"], comp_s3_s3t_df["dse"]).cdf(0)
comp_s3_s3t_df.round(3)
```
...
comp_st_df = comp_st[["elpd_diff", "dse"]].copy()
comp_st_df["p_worse"] = pz.Normal(comp_st_df["elpd_diff"], comp_st_df["dse"]).cdf(0)
...
idata_r1 = fit_r1.fit(**bambi_kwargs)
...
idata_r2 = fit_r2.fit(**bambi_kwargs)
...
idata_r3 = fit_r3.fit(**bambi_kwargs)
```
...
comp_roaches_df = comp_roaches[["elpd_diff", "dse"]].copy()
comp_roaches_df["p_worse"] = pz.Normal(comp_roaches_df["elpd_diff"], comp_roaches_df["dse"]).cdf(0)
comp_roaches_df.round(2)
```
...
#| cache: true
fit_r4 = bmb.Model(
    "y ~ bs(sqrt_roach1, df=6) + treatment + senior + offset(log(exposure2))",
    roaches,
    family="zero_inflated_negativebinomial",
    link="log",
    priors={
...
idata_r4 = fit_r4.fit(**bambi_kwargs)
```

```{python}
comp_r3_r4 = az.compare({"M_4 (ZINB+spline)": idata_r4, "M_3 (ZINB)": idata_r3})
comp_r3_r4_df = comp_r3_r4[["elpd_diff", "dse"]].copy()
comp_r3_r4_df["p_worse"] = pz.Normal(comp_r3_r4_df["elpd_diff"], comp_r3_r4_df["dse"]).cdf(0)
comp_r3_r4_df.round(2)
```
