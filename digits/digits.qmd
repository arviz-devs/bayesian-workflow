...
The following code centers the covariate to reduce posterior
...
We run MCMC with the current default settings. All convergence
...
mcmc.run(random.PRNGKey(SEED), **data_lin_priors)
dt_fit_lin = az.from_numpyro(mcmc)
```
...
xr.concat(
    ...
    dim="mcse",
).assign_coords(mcse=["mcse_mean", "mcse_q05", "mcse_q95"]).to_dataframe().T.round(3)
```
